...
def SavePloat_Voxels(voxels, voxels_real, path, iteration):
    slice_index = 32
    ...
    plt.savefig(path + '/{}.png'.format(str(iteration).zfill(5)))
    plt.close()
    ...
    # 绘制XY平面的切片
    axes[0].imshow(voxels_real[:, :, slice_index], cmap='hot')
    axes[0].set_title(f'XY Plane at Slice {slice_index}')

    # 绘制XZ平面的切片
    axes[1].imshow(voxels_real[:, slice_index, :], cmap='hot', aspect='auto')
    axes[1].set_title(f'XZ Plane at Slice {slice_index}')

    # 绘制YZ平面的切片
    axes[2].imshow(voxels_real[slice_index, :, :], cmap='hot', aspect='auto')
    axes[2].set_title(f'YZ Plane at Slice {slice_index}')

    plt.savefig(path + '/{}gt.png'.format(str(iteration).zfill(5)))
    plt.close()
