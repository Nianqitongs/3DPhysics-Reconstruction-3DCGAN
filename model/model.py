...
class net_G(torch.nn.Module):
    def __init__(self, args):
        ...
        # -------------------------------------
        self.layer = self.conv_layer(403, self.f_dim * 16, kernel_size=4, stride=2, padding=1, bias=self.bias)
        # -------------------------------------
        self.layer1 = self.conv_layer(self.f_dim * 16, self.f_dim * 8, kernel_size=4, stride=2, padding=1, bias=self.bias)
    ...
    def forward(self, out):
        ...
        # out = torch.squeeze(out)#(32,64,64,64)
        return out
...
class net_D(torch.nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.cube_len = 64  # params.cube_len
        ...
        self.layer1 = self.conv_layer(4, self.f_dim, kernel_size=4, stride=2, padding=(1, 1, 1), bias=self.bias)  # 这里的2是4
        ...
        self.layer6 = torch.nn.Sequential(
            torch.nn.Conv3d(self.f_dim * 16, 1, kernel_size=4, stride=2, bias=self.bias, padding=1),
            torch.nn.Sigmoid()
        )
